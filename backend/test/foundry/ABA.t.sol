...
contract ABATest is TestHelperOz5 {
    ...
    function setUp() public virtual override {
        ...
        flareAdaptor = FlareAdaptor(
            payable(_deployOApp(type(FlareAdaptor).creationCode, abi.encode()))
        );
    ...
    function test_combine_options() public {
        ...
        aEnforcedOptions[0] = EnforcedOptionParam({eid: bEid, msgType: REQUEST_READ, options: OptionsBuilder.newOptions().addExecutorLzReceiveOption(50000, 0)});
        aEnforcedOptions[1] = EnforcedOptionParam({eid: bEid, msgType: RETURN_DATA, options: OptionsBuilder.newOptions().addExecutorLzReceiveOption(500000, 0)});
        
        EnforcedOptionParam[] memory bEnforcedOptions = new EnforcedOptionParam[](2);
        bEnforcedOptions[0] = EnforcedOptionParam({eid: aEid, msgType: REQUEST_READ, options: OptionsBuilder.newOptions().addExecutorLzReceiveOption(50000, 0)});
        bEnforcedOptions[1] = EnforcedOptionParam({eid: aEid, msgType: RETURN_DATA, options: OptionsBuilder.newOptions().addExecutorLzReceiveOption(50000, 0)});
        ...
        MessagingFee memory returnFee = bReceiver.quote(aEid, REQUEST_READ, "Remote chain says hello!", _extraReturnOptions, OptionsBuilder.newOptions(), false);
    ...
    function test_aba() public {
        ...
        aEnforcedOptions[0] = EnforcedOptionParam({eid: bEid, msgType: REQUEST_READ, options: OptionsBuilder.newOptions().addExecutorLzReceiveOption(50000, 0)}); // gas limit, msg.value
        // Send gas for lzReceive + msg.value for nested lzSend (A -> B -> A).
        aEnforcedOptions[1] = EnforcedOptionParam({eid: bEid, msgType: RETURN_DATA, options: OptionsBuilder.newOptions().addExecutorLzReceiveOption(600000, 200000)});
        
        EnforcedOptionParam[] memory bEnforcedOptions = new EnforcedOptionParam[](2);
        // Send gas for lzReceive (A -> B).
        bEnforcedOptions[0] = EnforcedOptionParam({eid: aEid, msgType: REQUEST_READ, options: OptionsBuilder.newOptions().addExecutorLzReceiveOption(50000, 0)});
        // Send gas for lzReceive (B -> A).
        bEnforcedOptions[1] = EnforcedOptionParam({eid: aEid, msgType: RETURN_DATA, options: OptionsBuilder.newOptions().addExecutorLzReceiveOption(600000, 200000)});
        ...
        // Quote the return call B -> A.
        MessagingFee memory returnFee = bReceiver.quote(aEid, REQUEST_READ, "Request Data", _extraReturnOptions, OptionsBuilder.newOptions(), false);
        ...
        MessagingFee memory sendFee = aSender.quote(bEid, RETURN_DATA, "Request Data", _extraSendOptions, _extraReturnOptions, false);
        ...
        string memory expectedReturnMessage = string(abi.encodePacked("Value: ", expectedValue, ", Timestamp: ", expectedTimestamp));
